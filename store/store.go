...
func (api *Store) EnableThrottling(rate time.Duration, capacity int64) {
	api.bucket = tokenbucket.NewBucket(rate, capacity)
}

func (api *Store) DisableThrottling() {
	api.bucket = nil
}

func (api *Store) SetDelay(rate time.Duration) {
	api.bucket.SetRate(rate)
}

func (api *Store) GetDelay() time.Duration {
...
func (api Store) execQuery(urlStr string, form url.Values, data interface{}) error {
